...
file = matopen("local/eldredge/results/force_jl.mat", "r")
t_jl = read(file, "t")
CL_jl = read(file, "CL")
CD_jl = read(file, "CD")
close(file)

data = readdlm("local/eldredge/results/force_cpp1.dat")
t_cpp1 = data[:, 2]
#CD_cpp = data[:, 3]
CL_cpp1 = data[:, 4]

# C++ Body-fixed
data = readdlm("local/eldredge/results/force_cpp2.dat")
t_cpp2 = data[:, 2]
#CD_cpp = data[:, 3]
F = data[:, 3:4]
...
data = readdlm("local/eldredge/results/force_fort.dat")
t_fort = data[:, 1]*1e-3
CD_fort = data[:, 4]
CL_fort = data[:, 5]

plot(t_cpp2, CL_cpp2, lw=2, label="C++", xlims=(0, 7),
    xlabel=L"$t$", ylabel=L"$C_L$", size=(600, 300))
#plot!(t_cpp2, CL_cpp2, lw=2, label="C++ (2)")
plot!(t_fort, CL_fort, lw=2, label="Fortran")
plot!(t_jl, CL_jl, color=:black, ls=:dash, lw=2, label="Julia")
