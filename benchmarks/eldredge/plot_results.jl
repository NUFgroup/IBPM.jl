...
file = matopen("benchmarks/eldredge/force_jl1.mat", "r")
t_jl = read(file, "t")
CL_jl = read(file, "CL")
CD_jl = read(file, "CD")
close(file)
...
data = readdlm("benchmarks/eldredge/force_cpp1.dat")
t_cpp1 = data[:, 2]
#CD_cpp = data[:, 3]
CL_cpp1 = data[:, 4]

# C++ Body-fixed
data = readdlm("benchmarks/eldredge/force_cpp2.dat")
t_cpp2 = data[:, 2]
#CD_cpp = data[:, 3]
F = data[:, 3:4]
...
data = readdlm("benchmarks/eldredge/force_fort.dat")
t_fort = data[:, 1]*1e-3
CD_fort = data[:, 4]
CL_fort = data[:, 5]
