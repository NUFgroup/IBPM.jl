...
# To visualize
anim = @time animated_sim(t, F, state, prob) #advance to final time
gif(anim, "benchmarks/eldredge/results/eldredge_motion.gif", fps = 30)

CD = F[:, 1]
CL = F[:, 2]
matwrite("benchmarks/eldredge/results/force_jl1.mat", Dict(
	"t" => Array(t),
	"CL" => CL,
	"CD" => CD,
))
