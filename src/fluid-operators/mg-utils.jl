...
function coarsify!(Γc, Γ, grid::MultiGrid)
    ...
    for (j, jc) ∈ zip( ny÷2 .+ ( (-ny÷2+2):2:(ny÷2-2) ),
                       ny÷2 .+ ( (-ny÷4+1):(ny÷4-1) ) )
        for (i, ic) ∈ zip( nx÷2 .+ ( (-nx÷2+2):2:(nx÷2-2) ),
                           nx÷2 .+ ( (-nx÷4+1):(nx÷4-1) ) )
           Γc[ic, jc] = Γ[i, j] +
    ...
    """
    # Alternative: slightly slower but avoids for loops
    #  216.517 μs (10 allocations: 154.39 KiB)
