...
x0, y0 = 0.0, 0.5
xc(t) = [-cos(t), 0, 0]  # (x, y, θ)
uc(t) = [sin(t), 0, 0]     # (ẋ, ẏ, θ̇)
...
prob = ibpm.IBProblem(grid, bodies, Δt, Re; Uinf=0.0);
...
anim = ibpm.animated_sim(t, state, prob; output=10, nplt=100) do state, prob
        ibpm.plot_state(state, prob.model.grid, clims=(-5, 5))  # Plot vorticity
        plot_body(prob.model.bodies[1])
end
gif(anim, "examples/osc_plate_motion.gif", fps=30)
